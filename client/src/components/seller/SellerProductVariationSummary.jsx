...
export default function SellerProductVariationSummary({
  ...
  const hasBarcode = normalizedSummary.variants.some((entry) => entry.barcode);
  const hasImage = normalizedSummary.variants.some((entry) => entry.image);
  ...
  return (
    <div className="space-y-4">
      ...
      {showAttributeValues && normalizedSummary.attributes.length > 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  <th className={tableHeadClass}>SR</th>
                  {hasImage ? <th className={tableHeadClass}>Image</th> : null}
                  <th className={tableHeadClass}>Combination</th>
                  {hasSku ? <th className={tableHeadClass}>SKU</th> : null}
                  {hasBarcode ? <th className={tableHeadClass}>Barcode</th> : null}
                  {hasPrice ? <th className={tableHeadClass}>Original Price</th> : null}
                  {hasSalePrice ? <th className={tableHeadClass}>Sale Price</th> : null}
                  {hasQuantity ? <th className={tableHeadClass}>Quantity</th> : null}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 bg-white">
                {normalizedSummary.variants.map((variant, index) => (
                  <tr key={variant.id}>
                    ...
                    {hasBarcode ? (
                      <td className={tableCellClass}>{variant.barcode || "-"}</td>
                    ) : null}
                    {hasPrice ? (
                      <td className={tableCellClass}>
