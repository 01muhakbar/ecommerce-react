...
export const normalizeStorefrontProduct = (rawProduct: any) => {
  ...
  const rawPurchaseState =
    rawProduct?.purchaseState && typeof rawProduct.purchaseState === "object"
      ...
      : null;
  const rawStore =
    rawProduct?.store && typeof rawProduct.store === "object" ? rawProduct.store : null;
  const storeId = toNumberOrNull(rawProduct?.storeId ?? rawStore?.id);
  const storeSlug = toText(rawProduct?.storeSlug ?? rawStore?.slug) || null;
  ...
  return normalizeProduct({
    ...
    store: rawStore
      ? {
          id: storeId,
          name: toText(rawStore.name) || null,
          slug: storeSlug,
          status: toText(rawStore.status) || null,
        }
      : rawProduct?.store ?? null,
...
export const normalizeStorefrontProductsResponse = (payload: any) => {
  ...
  const items = rawItems
    ...
    .filter(Boolean);
  logProductNormalization("storefront.products.list", rawItems, items);
...
export const normalizeStorefrontProductDetailResponse = (payload: any) => {
  ...
  logProductNormalization("storefront.products.detail", root, normalized);
  return {
    ...payload,
    data: {
      ...(root && typeof root === "object" && !Array.isArray(root) ? root : {}),
