...
import { logProductNormalization } from "../services/adapters/productAdapter.js";
...
export const getSellerProducts = async (
  ...
  const normalizedItems = rawItems.map(normalizeProductListItem).filter(Boolean);
  logProductNormalization("seller.products.list", rawItems, normalizedItems);
...
export const getSellerProductDetail = async (
  ...
  const rawProduct = data?.data ?? null;
  const normalizedProduct = normalizeProductDetail(rawProduct);
  logProductNormalization("seller.products.detail", rawProduct, normalizedProduct);
