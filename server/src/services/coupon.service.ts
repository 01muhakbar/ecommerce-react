...
const warnCouponOwnership = (payload: Record<string, unknown>) => {
  // Keep coupon ownership issues observable without changing coupon validity flow.
  // eslint-disable-next-line no-console
  console.warn("coupon-ownership-warning", payload);
};

const warnCouponPolicy = (payload: Record<string, unknown>) => {
  // Surface legacy-vs-modern coupon policy drift without breaking live checkouts.
  // eslint-disable-next-line no-console
  console.warn("coupon-policy-warning", payload);
};
...
export type CouponQuoteResult = {
  ...
  shipping: number;
  total: number;
  policyWarnings?: CouponCheckoutPolicyWarning[];
};

export type CouponCheckoutPolicyWarning = {
  code: "platform_multi_store_legacy";
  message: string;
  couponScopeType: "PLATFORM" | "STORE";
  couponStoreId: number | null;
  checkoutStoreIds: number[];
  recommendedFutureAction: "DENY_PLATFORM_MULTI_STORE";
};

export type CouponCheckoutPolicyResult = {
  allowed: boolean;
  reason?: CouponQuoteReason;
  message?: string;
  ...
  storeId: number | null;
  checkoutStoreIds: number[];
  warnings: CouponCheckoutPolicyWarning[];
};
...
export const canApplyCouponToCheckout = (
  coupon: any,
  context?: CouponScopeContext
): CouponCheckoutPolicyResult => {
  const { scopeType, storeId } = resolveCouponGovernanceScope(coupon);
  const normalizedStoreIds = normalizeStoreIds(context);

  if (scopeType !== "STORE") {
    const warnings: CouponCheckoutPolicyWarning[] = [];
    if (
      context?.policySurface === "LEGACY_ORDER" &&
      normalizedStoreIds.length > 1
    ) {
      warnings.push({
        code: "platform_multi_store_legacy",
        message:
          "Platform coupon is being applied to a multi-store legacy checkout path.",
        couponScopeType: scopeType,
        couponStoreId: storeId,
        checkoutStoreIds: normalizedStoreIds,
        recommendedFutureAction: "DENY_PLATFORM_MULTI_STORE",
      });
    }
    return {
...
export const quoteCoupon = async (
  ...
  const policy = canApplyCouponToCheckout(coupon, context);
  if (policy.warnings.length > 0) {
    policy.warnings.forEach((warning) => {
      warnCouponPolicy({
        couponId,
        code: effectiveCode,
        policySurface: context?.policySurface || null,
        couponScopeType: warning.couponScopeType,
        couponStoreId: warning.couponStoreId,
        checkoutStoreIds: warning.checkoutStoreIds,
        itemCount:
          Number.isFinite(Number(context?.itemCount)) && Number(context?.itemCount) >= 0
            ? Number(context?.itemCount)
            : null,
        subtotal,
        recommendedFutureAction: warning.recommendedFutureAction,
  ...
  if (!policy.allowed) {
    warnCouponOwnership({
      ...
      couponId,
      code: effectiveCode,
      couponScopeType: policy.scopeType,
      couponStoreId: policy.storeId,
      checkoutStoreIds: policy.checkoutStoreIds,
    });
    return buildBaseQuote(effectiveCode, subtotal, shipping, {
      reason: policy.reason,
...
export const validateCoupon = async (
  ...
  if (!quoted.valid) {
    ...
    return {
      ...
      policyWarnings: quoted.policyWarnings || [],
    };
  }
