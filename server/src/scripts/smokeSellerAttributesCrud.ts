...
const buildSellerVariationPayload = (input: {
  attributeId: number;
  ...
  hasVariants: true,
  selectedAttributes: [{ id: input.attributeId, name: input.attributeName }],
  selectedAttributeValues: [
    {
      attributeId: input.attributeId,
      values: [{ id: input.valueId, label: input.value, value: input.value }],
    },
  ],
  variants: [
    {
      id: "variant-1",
      combination: `${input.attributeName}: ${input.value}`,
      combinationKey: `${input.attributeId}:${String(input.valueId).toLowerCase()}`,
      selections: [
        {
          attributeId: input.attributeId,
...
async function main() {
  ...
  const globalValues = await findAttributeValueRows(globalPublishedId);
  const globalValueId = Number(globalValues[0]?.id || 0);
  assert.ok(globalValueId > 0, "published global attribute value missing");
  const storeValues = await findAttributeValueRows(createdAttributeId);
  const storeValueId = Number(storeValues[0]?.id || 0);
  assert.ok(storeValueId > 0, "store attribute value missing");

  logStep("seller product save rejects mismatched attribute value pairs");
  const mismatchProductResponse = await sellerClient.request(
    `/api/seller/stores/${sellerStore.id}/products/drafts`,
    {
      method: "POST",
      body: JSON.stringify({
        name: `${RUN_ID}-mismatch-product`,
        price: 15000,
        stock: 5,
        hasVariants: true,
        variations: buildSellerVariationPayload({
          attributeId: createdAttributeId,
          attributeName: "Store Material Updated",
          valueId: globalValueId,
          value: String(globalValues[0]?.value || "alpha"),
        }),
      }),
    }
  );
  assertStatus(mismatchProductResponse, 400, "seller product mismatched attribute value");
  assert.equal(
    String(mismatchProductResponse.body?.code || ""),
    "ATTRIBUTE_VALUE_MISMATCH",
    "mismatched attribute value should be rejected with ATTRIBUTE_VALUE_MISMATCH"
  );
  logPass("seller product save rejects mismatched attribute value pairs");

  logStep("seller product save rejects inactive attribute values");
  await sequelize.query("UPDATE attribute_values SET status = 'archived' WHERE id = ?", {
    replacements: [storeValueId],
  });
  const inactiveProductResponse = await sellerClient.request(
    `/api/seller/stores/${sellerStore.id}/products/drafts`,
    {
      method: "POST",
      body: JSON.stringify({
        name: `${RUN_ID}-inactive-value-product`,
        price: 15000,
        stock: 5,
        hasVariants: true,
        variations: buildSellerVariationPayload({
          attributeId: createdAttributeId,
          attributeName: "Store Material Updated",
          valueId: storeValueId,
          value: String(storeValues[0]?.value || "Cotton"),
        }),
      }),
    }
  );
  assertStatus(inactiveProductResponse, 400, "seller product inactive attribute value");
  assert.equal(
    String(inactiveProductResponse.body?.code || ""),
    "ATTRIBUTE_VALUE_INACTIVE",
    "inactive attribute value should be rejected with ATTRIBUTE_VALUE_INACTIVE"
  );
  await sequelize.query("UPDATE attribute_values SET status = 'active' WHERE id = ?", {
    replacements: [storeValueId],
  });
  logPass("seller product save rejects inactive attribute values");
