...
const toOrderDate = (daysAgo: number, hour: number) => {
  const date = new Date();
...
async function seedAnalyticsDemo() {
  ...
  try {
    ...
    const demoUserIds = demoUsers.map((user: any) => user.id);
    ...
    const demoOrderIds = demoOrders.map((order: any) => order.id);
    ...
    for (let day = 0; day < 7; day += 1) {
      for (let i = 0; i < 2; i += 1) {
        ...
        const productB =
          createdProducts[(day + i + 2) % createdProducts.length];
        const items = [
        ...
        for (const item of items) {
          ...
          await queryInterface.bulkInsert(
            OrderItem.getTableName() as string,
            [
              {
