...
const toSafePublicStoreSettings = (settings: ReturnType<typeof sanitizeStoreSettings>) => ({
  payments: {
    cashOnDeliveryEnabled: Boolean(settings.payments.cashOnDeliveryEnabled),
    stripeEnabled: Boolean(settings.payments.stripeEnabled),
    razorPayEnabled: Boolean(settings.payments.razorPayEnabled),
    stripeKey: toText(settings.payments.stripeKey, ""),
  },
  socialLogin: {
    googleEnabled: Boolean(settings.socialLogin.googleEnabled),
    githubEnabled: Boolean(settings.socialLogin.githubEnabled),
    facebookEnabled: Boolean(settings.socialLogin.facebookEnabled),
    googleClientId: toText(settings.socialLogin.googleClientId, ""),
    githubId: toText(settings.socialLogin.githubId, ""),
    facebookId: toText(settings.socialLogin.facebookId, ""),
  },
  analytics: {
    googleAnalyticsEnabled: Boolean(settings.analytics.googleAnalyticsEnabled),
    googleAnalyticKey: toText(settings.analytics.googleAnalyticKey, ""),
  },
  chat: {
...
const getPersistedStoreSettings = async () => {
  ...
  if (!row?.value) return sanitizeStoreSettings({});
  try {
...
router.get("/", async (_req, res, next) => {
  try {
    ...
    const storeSettings = toSafePublicStoreSettings(persisted);
    return res.json({
      success: true,
      data: { storeSettings },
    });
