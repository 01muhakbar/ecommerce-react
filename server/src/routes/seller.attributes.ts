...
import { resolveAttributeValueVariationUsage } from "../services/attributeValueVariationUsage.js";
...
const resolveAttributeValueUsage = async (valueIds: readonly number[]) => {
  const [legacyUsage, variationValueUsage] = await Promise.all([
    loadLegacyUsageMap("attribute_value_id", valueIds),
    resolveAttributeValueVariationUsage(valueIds),
  ]);
  ...
  valueIds.forEach((valueId) => {
    ...
    (variationValueUsage.get(valueId) || []).forEach((productId) => merged.add(productId));
