...
export const createProduct = async (req: CustomRequest, res: Response): Promise<void> => {
  try {
    const {
      name, description, price, stock, categoryId, status, gtin, notes,
      parentSku, condition, weight, length, width, height, dangerousProduct,
      preOrder, preorderDays, youtubeLink, variations, wholesale,
    } = req.body;
    ...
    if (req.files && (req.files as { [fieldname: string]: Express.Multer.File[] }).promoProductImage) {
      promoImagePath = (req.files as { [fieldname: string]: Express.Multer.File[] }).promoProductImage[0].path;
    }

    let imagePaths: string[] = [];
    if (req.files && (req.files as { [fieldname: string]: Express.Multer.File[] }).productImages) {
      imagePaths = (req.files as { [fieldname: string]: Express.Multer.File[] }).productImages.map((file: Express.Multer.File) => file.path);
    }

    let videoPath: string | undefined = undefined;
    if (req.files && (req.files as { [fieldname: string]: Express.Multer.File[] }).productVideo) {
      videoPath = (req.files as { [fieldname: string]: Express.Multer.File[] }).productVideo[0].path;
    }
...
export const getProductById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: "Failed to fetch product details", error: (error as Error).message });
  }
};

// [REFACTORED] Get all products for the logged-in seller
export const getSellerProducts = async (req: CustomRequest, res: Response): Promise<void> => {
  try {
    if (!req.user?.id) {
    ...
    res.status(500).json({ message: "Error loading products", error: (error as Error).message });
  }
};

// [REFACTORED] Get product data for the edit page
export const getEditProductPage = async (req: CustomRequest, res: Response): Promise<void> => {
  try {
    if (!req.user?.id) {
      res.status(401).json({ message: "Unauthorized" });
      return;
    ...
    res.status(500).json({ message: "Error loading product for editing", error: (error as Error).message });
  }
};

// [REFACTORED] Get product and categories data for admin edit page
export const renderAdminEditProductPage = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.query;
...
export const renderAdminProductsPage = async (req: Request, res: Response): Promise<void> => {
  try {
    const categories = await Category.findAll();
    ...
    res.status(500).json({ message: "Error loading page data", error: (error as Error).message });
  }
};

// [REFACTORED] Get categories for the add product page
export const renderAddProductPageAdmin = async (req: Request, res: Response): Promise<void> => {
  try {
    const categories = await Category.findAll();
...
export const getProductDetailsForPreview = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const product = await Product.findByPk(id, {
      include: [
...
export const renderAllProducts = async (req: Request, res: Response): Promise<void> => {
  try {
    const products = await Product.findAll({
      order: [['createdAt', 'DESC']],
    });
    ...
    res.status(500).json({ message: "Error memuat halaman produk", error: (error as Error).message });
  }
};
