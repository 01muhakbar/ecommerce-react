...
const toActiveSalePrice = (price, salePrice) => {
    if (!Number.isFinite(Number(salePrice)))
        return null;
    const normalized = Number(salePrice);
    if (normalized <= 0 || normalized >= Number(price))
        return null;
    return normalized;
};
async function seedKachaDemo() {
    ...
    for (const category of CATEGORIES) {
        ...
        if (existing) {
            const shouldUpdate = existing.name !== category.name ||
    ...
    for (const product of PRODUCTS) {
        ...
        if (existing) {
            ...
            seededProducts.push({ id: existing.id, slug: product.slug, ratings: product.ratings });
            productsUpdated += 1;
            continue;
        }
        ...
        seededProducts.push({ id: created.id, slug: product.slug, ratings: product.ratings });
        productsCreated += 1;
    }
    const reviewerIds = [];
