...
import { createOrderSchema, reviewCreateSchema, reviewUpdateSchema, } from "@ecommerce/schemas";
import { Category, Order, OrderItem, Product, ProductReview, User, sequelize, } from "../models/index.js";
...
const getAttr = (row, key) => row?.getDataValue?.(key) ??
    ...
    row?.dataValues?.[key] ??
    undefined;
const toCanonicalOrderStatus = (raw) => {
...
function normalizeUploadsUrl(v) {
    if (!v)
        return null;
    if (/^https?:\/\//i.test(v))
        return v;
...
function getProductId(product) {
    const raw = product?.getDataValue?.("id") ??
        product?.get?.("id") ??
        product?.id;
    return Number(raw);
}
...
const toReviewResponse = (review, product) => {
    const images = review?.images ?? review?.get?.("images") ?? null;
    const createdAt = review?.createdAt ?? review?.get?.("createdAt") ?? review?.created_at ?? null;
    const updatedAt = review?.updatedAt ?? review?.get?.("updatedAt") ?? review?.updated_at ?? null;
    return {
...
const toProductListItem = (product) => {
    ...
    const hasDiscount = Number.isFinite(Number(salePrice)) &&
        Number(salePrice) > 0 &&
...
router.get("/categories", async (_req, res, next) => {
    try {
        ...
        const normalized = categories
            ...
            const code = String(getAttr(category, "code") ?? getAttr(category, "slug") ?? "").trim();
            const name = String(getAttr(category, "name") ??
                getAttr(category, "title") ??
                getAttr(category, "label") ??
                code).trim();
            const parentId = getAttr(category, "parentId") ?? getAttr(category, "parent_id") ?? null;
            const publishedRaw = getAttr(category, "published") ?? getAttr(category, "isPublished") ?? true;
            const imageRaw = getAttr(category, "icon") ??
    ...
    try {
        ...
        const { rows, count } = await Product.findAndCountAll({
            ...
            attributes: [
                ...
                "updatedAt",
                [
                    sequelize.literal("(SELECT ROUND(AVG(pr.rating), 1) FROM product_reviews pr WHERE pr.product_id = Product.id)"),
                    "ratingAvg",
                ],
                [
                    sequelize.literal("(SELECT COUNT(*) FROM product_reviews pr WHERE pr.product_id = Product.id)"),
                    "reviewCount",
                ],
            ],
        ...
        if (process.env.NODE_ENV !== "production") {
            console.log("[store/products] sample", rows?.[0]?.toJSON?.() ?? rows?.[0]);
        }
        res.json({
...
router.get("/my/orders", protect, async (req, res, next) => {
    try {
        ...
        if (!userId)
            return;
        const [rows] = await sequelize.query("SELECT id, invoice_no, status, total_amount, created_at, payment_method FROM orders WHERE user_id = ? ORDER BY created_at DESC LIMIT 20", { replacements: [userId] });
        res.json({
            data: rows.map((row) => ({
                id: row.id,
...
router.get("/orders/:ref", async (req, res, next) => {
    try {
        ...
        const createdAt = order.createdAt ??
            ...
            null;
        const [rows] = await sequelize.query("SELECT oi.product_id, oi.quantity, oi.price, p.name FROM orderitems oi LEFT JOIN products p ON p.id=oi.product_id WHERE oi.order_id = ?", { replacements: [orderId] });
...
router.get("/my/reviews", protect, async (req, res, next) => {
    try {
        ...
        const data = reviews.map((review) => toReviewResponse(review, review?.product ?? review?.get?.("product")));
...
router.patch("/reviews/:id", protect, async (req, res, next) => {
    try {
...
router.put("/reviews/product/:productId", protect, async (req, res, next) => {
    try {
...
router.post("/orders", protect, async (req, res, next) => {
    try {
        ...
        for (const item of items) {
            normalizedItems.set(item.productId, (normalizedItems.get(item.productId) || 0) + item.qty);
        }
        ...
        try {
            ...
            if (process.env.NODE_ENV !== "production") {
                ...
                console.log("[store/orders] sampleProduct", products?.[0]?.toJSON?.() ?? products?.[0]);
            }
            const foundIdsArr = products
                .map((product) => getProductId(product))
                .filter((n) => Number.isFinite(n) && n > 0);
            ...
            const getUnitPrice = (product) => {
                const rawSale = product?.getDataValue?.("salePrice") ??
                    product?.get?.("salePrice") ??
                    product?.salePrice;
                const rawPrice = product?.getDataValue?.("price") ??
                    product?.get?.("price") ??
                    product?.price;
                const rawOriginal = product?.getDataValue?.("originalPrice") ??
            ...
            for (const item of itemsNorm) {
                ...
                const stock = Number(product.getDataValue?.("stock") ?? product.stock ?? 0);
                if (stock < item.qty) {
                    await tx.rollback();
                    return res.status(409).json({
                        message: "Insufficient stock",
                        data: {
                            productId: Number(product.getDataValue?.("id") ?? product.id),
                            name: String(product.getDataValue?.("name") ?? product.name ?? ""),
                            available: stock,
                            requested: item.qty,
                        },
                    });
            ...
            for (const item of itemsNorm) {
                const product = byId.get(item.productId);
                const currentStock = Number(product.getDataValue?.("stock") ?? product.get?.("stock") ?? product.stock ?? 0);
                product.stock = currentStock - item.qty;
                await product.save({ transaction: tx });
            }
            await tx.commit();
            const invoiceNo = order?.getDataValue?.("invoiceNo") ??
                order?.get?.("invoiceNo") ??
                order?.dataValues?.invoiceNo ??
                order?.invoiceNo ??
                null;
