...
router.get("/", async (_req, res, next) => {
    try {
        const rows = await sequelize.query("SELECT id, name FROM attributes ORDER BY name ASC", { type: QueryTypes.SELECT });
...
router.post("/", async (req, res, next) => {
    try {
        ...
        try {
            const [_rows, meta] = await sequelize.query("INSERT INTO attributes (name, created_at, updated_at) VALUES (?, NOW(), NOW())", { replacements: [name] });
            const id = meta?.insertId ?? null;
...
router.delete("/:id", async (req, res, next) => {
    try {
        ...
        try {
            const [_rows, meta] = await sequelize.query("DELETE FROM attributes WHERE id = ?", { replacements: [id] });
