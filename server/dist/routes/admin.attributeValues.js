...
router.get("/attributes/:id/values", async (req, res, next) => {
    try {
        ...
        const rows = await sequelize.query("SELECT id, attribute_id, value FROM attribute_values WHERE attribute_id = ? ORDER BY value ASC", { replacements: [attributeId], type: QueryTypes.SELECT });
...
router.post("/attributes/:id/values", async (req, res, next) => {
    try {
        ...
        try {
            const [_rows, meta] = await sequelize.query("INSERT INTO attribute_values (attribute_id, value, created_at, updated_at) VALUES (?, ?, NOW(), NOW())", { replacements: [attributeId, value] });
            let id = meta?.insertId ??
                meta?.[0]?.insertId ??
                null;
            if (!id) {
                const rows = await sequelize.query("SELECT id FROM attribute_values WHERE attribute_id = ? AND value = ? LIMIT 1", { replacements: [attributeId, value], type: QueryTypes.SELECT });
                id = rows?.[0]?.id ?? null;
            }
            return res.json({
                success: true,
...
router.delete("/attribute-values/:id", async (req, res, next) => {
    try {
        ...
        try {
            const [_rows, meta] = await sequelize.query("DELETE FROM attribute_values WHERE id = ?", { replacements: [id] });
            const affected = Number(meta?.affectedRows || 0);
            if (affected === 0) {
                return res
                    .status(404)
