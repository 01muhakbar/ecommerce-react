...
const getCouponField = (coupon, keys) => {
    if (!coupon)
        return undefined;
    const raw = typeof coupon.get === "function" ? coupon.get({ plain: true }) : coupon;
...
const resolveDiscountType = (value) => {
    const raw = String(value || "").trim().toLowerCase();
    return raw === "fixed" ? "fixed" : "percent";
};
export const normalizeCouponRecord = (coupon) => {
    const code = normalizeCode(getCouponField(coupon, ["code", "couponCode", "coupon_code"]));
    const discountType = resolveDiscountType(getCouponField(coupon, ["discountType", "discount_type", "type"]));
    const amount = parseLocaleNumber(getCouponField(coupon, [
        "amount",
        "discountAmount",
        "discount_amount",
        "discount",
    ...
    const minSpend = parseLocaleNumber(getCouponField(coupon, [
        "minSpend",
        "min_spend",
        "minimumSpend",
        "minimum_spend",
...
export const validateCoupon = async (codeRaw, subtotalRaw) => {
    ...
    const { code: resolvedCode, discountType, amount, minSpend } = normalizeCouponRecord(coupon);
