...
const explicitEnvFile = String(
  process.env.PUBLIC_RELEASE_SMOKE_ENV_FILE || process.env.PUBLIC_RELEASE_ENV_FILE || ""
).trim();

if (explicitEnvFile) {
  const result = dotenv.config({ path: explicitEnvFile, override: true });
  if (result.error) {
    throw new Error(
      `Failed to load public release env file at ${explicitEnvFile}: ${result.error.message}`
    );
  }
  console.log(`[public-release-smoke] LOAD env file ${explicitEnvFile}`);
}
...
const classifyDatabaseError = (env: CommandEnv, error: any) => {
  ...
  if (code === "ER_ACCESS_DENIED_ERROR") {
    return `DB readiness failed: access denied for ${describeDatabaseTarget(env)}. ${releaseDbSetupHint} (${details})`;
  }
  if (code === "ER_BAD_DB_ERROR") {
    return `DB readiness failed: database does not exist for ${describeDatabaseTarget(env)}. Verify DB_NAME or DATABASE_URL database path. ${releaseDbSetupHint} (${details})`;
  }
  if (code === "ENOTFOUND" || code === "EAI_AGAIN") {
    return `DB readiness failed: host could not be resolved for ${describeDatabaseTarget(env)}. Verify DB_HOST/DATABASE_URL host and network/DNS. ${releaseDbSetupHint} (${details})`;
  }
  if (code === "ECONNREFUSED" || code === "ETIMEDOUT") {
    return `DB readiness failed: database is not reachable at ${describeDatabaseTarget(env)}. Verify DB_HOST, DB_PORT, firewall, tunnel, or database service status. ${releaseDbSetupHint} (${details})`;
  }

  return `DB readiness failed for ${describeDatabaseTarget(env)}. Verify release DB env and connectivity. ${releaseDbSetupHint} (${details})`;
};
